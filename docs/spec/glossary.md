...
Domain vocabulary for reverse-engineering Roland **Sound Canvas VA** (`SCCore.dll`) into a static,
embeddable reimplementation of its synth engine — which is what this library is.

Written for a technically literate reader who does not necessarily know sampler internals or
Roland's house terminology. Every SC-VA-specific number here is quoted from
[FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md) or read off the
Python resolvers (`scvx_directory.py`, `scvx_partials.py`, `scvx_engine.py`) in the
[spec repository](https://github.com/TabulaSonora/spec). Where that document tags something
`[likely]` or `[guess]`, the uncertainty is carried through here. Addresses are absolute with image
base `0x180000000` unless stated as file offsets.

The names in this glossary are the vocabulary the API uses: a *partial* is
ts::PartialParameters, a *multisample* is ts::MultisampleZone, a *tone* is ts::Tone, and so on.
\ref signal-flow "The route of the sound" maps the rest.
...
In [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md)'s own words:
...
at `0x180061a40` walks a part's linked list of active partials/tones (list head at part-struct
...
with `LUT1` at `0x1819f2e30`, `LUT2` at `0x1819f28b0`, `LUT3` at `0x1819f32b0` (exported as
...
`tone_lookup` at `0x1800026d0`. Layout = 0x24 header + **2 partial blocks × 0x6e**. The tone **name
...
`multisample_select_wave` at `0x180003420`. Key-split upper bounds live at `+0x0c` (walk while
...
`.rdata 0x181897b40`, **stride 0x16**, decoded by `wavedesc_decode` at `0x18005ec90`. Fields:
...
**ROM bank** — Two base pointers, `g_wave_rom_base_a` at `0x181a18ef0` and `g_wave_rom_base_b`
at `0x181a11a68`, selected by `g_voice_wave_ctrl & 0x10`. As **file offsets** (validated
...
**Reverse playback** — A second codec/sampler variant (`dpcm_voice_init_rev` at `0x18003ff90`,
...
at `0x181a0f210` (128 phases × 4 float taps), indexed by `(phase_frac >> 9) * 0x10`; raw wavetable
samples are normalized by `2^-27` (`7.450581e-09`). [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md) calls this "the actual sauce" —
...
no separate ROM structure. Field offsets below are block-relative and come from [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md)'s
...
`tva_compute_base_level` at `0x180060960` and `tva_compute_env_levels` at `0x180060b00`; reimplemented
...
`+0x31` (values 0/1/2/4/5/6 index `g_filter_type_coef` at `0x181987b00`, 7×4 B, type 0 =
...
(0x40-centered). Computed by `partial_compute_filter` at `0x180061210`.
...
mapped through `g_reso_curve` at `0x1819a2b88` (`tables/curve_reso_2b88.bin`; index 8 flagged for
...
drops). Computed by `partial_compute_pitch_env` at `0x18005fde0`: depth `+0x18`, five stage biases
...
bend/LFO pitch depth (`partial_compute_pitch` at `0x18005fc20`). Not implemented in
`scvx_engine.py` yet.
...
**Envelope generator (the timing model)** — `env_ramp_segment` at `0x180083a70` is a **16-bit
...
per-unit rate steps. Chain: `control_tick_dispatch` at `0x18008f0d0` → `voices_control_update`
at `0x1800849a0` (64 voices, stride 0x220) → `voice_block_process` at `0x180080e40` →
`env_ramp_segment`.
...
(`lfo_advance_waveform` at `0x180082a30`; `lfo_update` at `0x180081b90` writes per-voice mod triples
`{pitch, TVF, TVA}` read at `voice+0x170/0x180/0x198`). Rate: `f = rate × 100 / 65536` with
`rate = g_lfo_rate_tbl[param]` at `0x1819a2790` — 0–20 Hz, a clean 0.1 Hz per unit up to ~8 Hz then
accelerating. Waveforms (`g_lfo_waveform_sel` at `0x181a227d0`): 0 sine, 1 random sample-and-hold,
2/3 slewed random, 4 square, 5 sawtooth, 6 triangle. Pitch depth via `g_lfo_cents_tbl`
at `0x1819a2690` = **10 cents/unit**, up to ±6000 cents. Mod wheel (CC1) adds on top of the part's
vibrato parameters. Caution: the function once labelled `lfo_value` at `0x18008fbb0` is actually a
...
`env_rate_scale` at `0x1800607e0` (base rate + modifier → rate multiplier, via `g_env_scale_curve`
and `g_env_rate_out`) and `env_level_scale` at `0x180060880` (same family for depths). Both are
...
under `tables/`:
- `g_env_rate_out` at `0x1819a3060` — an exact `2^((i − 0x80)/32)` exponential rate table
  (`[0x40]=2⁻²`, `[0x80]=2⁰=0x100`, `[0xa0]=2¹`, `[0xc0]=2²`).
- `g_level_curve` at `0x1819a2a00` — monotonic dB-style level → 16-bit log attenuation
...
- `g_filter_type_coef` at `0x181987b00`, `g_reso_curve` at `0x1819a2b88`, `g_env_shape`,
...
sounding partial. Voice state exists in two parallel forms: `g_voice_run_flags` at `0x181a1b5b8`
...
`FUN_18003f4e0` (sampler init) and `sample_fetch_loop_wrap` at `0x18003f870`. Note this **corrected
...
`SCCore.decompiled.c` (~2.7 MB). Important caveat from [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md)'s honesty preamble: variable
...
**Confidence tags** — [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md) marks each claim `[confirmed]` (the code plainly does this),
...
**Honest boundary** — A phrase used throughout [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md) to mark exactly where proof stops and
...
Carried forward from [FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md) so the glossary does not overstate:
...
- **Effect algorithms** — 67 `fx_algo_*` located and named via `g_fx_type_to_algo_map`
  at `0x18189566c`, but none dissected. Dispatch slot **66** (`fx_algo_orphan66_moddelay`
  at `0x180029c90`) is a complete, unreachable, modulated multi-tap delay; why it exists is `[guess]`.
...
[FINDINGS.md](https://github.com/TabulaSonora/spec/blob/main/docs/FINDINGS.md) and Python resolvers.
