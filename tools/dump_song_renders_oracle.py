...
SONGS = [
    ...
    # One note each, and here because the note sweep cannot reach what they reach: a tone whose
    # LFO1 is one of the three random shapes *and* which has two partials. The module claims one
    # LFO1 node per note refcounted by partial count, so those two partials share a phase and a
    # draw; twelve tones qualify, eight of them multi-partial, and not one is reachable from any of
    # the sweep's 185 melodic cases. `Stream` and `Bubble` are, at bank 4 and 5 of program 122.
    #
    # These six are ours and travel with the spec repository under `testdata/lfo/`, so unlike the
    # corpus above they can be regenerated by anyone. See \ref sharing-lfo1.
    ("lfo/randlfo1_stream_k48.mid", 1),
    ("lfo/randlfo1_stream_k60.mid", 1),
    ("lfo/randlfo1_stream_k72.mid", 1),
    ("lfo/randlfo1_bubble_k48.mid", 1),
    ("lfo/randlfo1_bubble_k60.mid", 1),
    ("lfo/randlfo1_bubble_k72.mid", 1),
]
